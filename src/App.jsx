...
const modules = [
  ...
  { id: 'accounting', icon: '৳', name: 'Accounting', line: 'Make every taka accountable.', image: '/assets/screenshots/Accounting.png', title: 'Accounting & Invoices', signal: 'Connected records', detail: 'Keep every invoice and commercial activity connected to the operational record.' },
...
function ArrowUpRight({ className = '' }) {
  return <svg className={`icon-arrow${className ? ` ${className}` : ''}`} width="14" height="14" style={{ width: 14, height: 14, minWidth: 14 }} viewBox="0 0 16 16" fill="none" aria-hidden="true" focusable="false"><path d="M4 12 12 4M6 4h6v6" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>;
}
function Brand() {
  return <a className="brand" href="#top" aria-label="Mayhem ERP home"><span className="brand-mark" aria-hidden="true"><img src="/assets/mayhem-logo.png" alt=""/></span><span><strong>MAYHEM</strong><small>ERP / BANGLADESH</small></span></a>;
}
...
function Header() {
  ...
  return <header className={`site-header${scrolled ? ' scrolled' : ''}${menuOpen ? ' menu-open' : ''}`}>
    <div className="shell nav-shell"><Brand/><nav className="desktop-nav" aria-label="Primary navigation"><a href="#demo">Product tour</a><a href="#platform">Platform</a><a href="#screens">Workspace</a><a href="#why-mayhem">Why Mayhem</a></nav><div className="nav-actions"><a className="text-link" href="#contact">Contact us</a><a className="button button-primary button-small" href="#contact">Request a demo</a></div><button className="menu-toggle" type="button" aria-label={menuOpen ? 'Close menu' : 'Open menu'} aria-expanded={menuOpen} onClick={() => setMenuOpen(value => !value)}><span/><span/></button></div>
    <nav className={`mobile-nav${menuOpen ? ' open' : ''}`} aria-label="Mobile navigation"><a href="#demo" onClick={close}>Product tour</a><a href="#platform" onClick={close}>Platform</a><a href="#screens" onClick={close}>Workspace</a><a href="#why-mayhem" onClick={close}>Why Mayhem</a><a href="#contact" onClick={close}>Contact us</a><a className="mobile-cta" href="#contact" onClick={close}>Request a demo</a></nav>
  </header>;
}

function Hero({ onVideo, onImage }) {
  return <><section className="hero section-grid" id="top"><div className="hero-orb hero-orb-one"/><div className="hero-orb hero-orb-two"/><div className="shell hero-layout"><div className="hero-copy reveal"><p className="eyebrow-pill"><span>✦</span> Operations, without the gaps</p><h1>The work is<br/><span className="gradient-text">connected now.</span></h1><p className="hero-lede">Mayhem ERP gives ambitious teams one calm control center for projects, production, inventory, people, customers, and the numbers that keep the lights on.</p><div className="button-row"><a className="button button-primary" href="#contact">See Mayhem in action <ArrowUpRight/></a><button className="button button-outline" type="button" onClick={onVideo}><span className="play-icon">▶</span> Watch the product tour</button></div></div><div className="hero-product reveal delay-one"><div className="product-glow"/><button className="product-frame" onClick={() => onImage('/assets/screenshots/war room .png', 'Mayhem ERP executive war room')} type="button"><div className="frame-bar"><span>Executive war room</span><span className="live"><i/> Live workspace</span></div><img src="/assets/screenshots/war room .png" alt="Mayhem ERP executive war room dashboard"/><span className="expand-chip">⌗ View larger</span></button><div className="pulse-card"><div><span>Workspace pulse</span><i/></div><strong>+28.4<small>%</small></strong><p>operations visibility</p><div className="bars"><i/><i/><i/><i/><i/><i/><i/></div></div></div></div><a className="scroll-cue" href="#demo"><span>Scroll to explore</span><b>↓</b></a></section><section className="discipline-strip"><div className="shell"><span className="mono-label">One operating language</span><div><span>Projects</span><span>Manufacturing</span><span>Inventory</span><span>Accounting</span><span>People</span><span>Customers</span></div></div></section></>;
}

function ProductTour() {
  return <section className="product-tour section-pad" id="demo"><div className="tour-glow"/><div className="shell">
    <div className="tour-heading reveal"><div><p className="section-kicker">01 / Product tour</p><h2>See the whole business<br/><span className="gradient-text">move as one.</span></h2></div><p>Start with the product itself. This walkthrough shows how Mayhem connects operations, records, and teams in one working system.</p></div>
    <div className="tour-stage reveal"><div className="tour-stage-bar"><span>Mayhem ERP / Product walkthrough</span><span className="live"><i/> Ready to play</span></div><video controls playsInline preload="metadata" poster="/assets/screenshots/war room .png" src="/assets/Mayhem_ERP_Product_Demo.mp4">Your browser does not support HTML video.</video></div>
    <div className="tour-notes reveal"><span>Projects</span><span>Manufacturing</span><span>Inventory</span><span>Accounting</span><span>People</span><span>Customers</span></div>
  </div></section>;
}
...
function Workspace() {
  return <section className="workspace section-pad" id="screens"><div className="shell workspace-shell">
    <div className="workspace-head reveal"><div><p className="section-kicker">03 / Inside the workspace</p><h2>Explore the<br/><span className="gradient-text">ERP data table.</span></h2></div><p>A live demo of the reusable table experience inside Mayhem ERP — search, filter, sort, edit and inspect sample records in one place.</p></div>
    <div className="datatable-stage reveal"><DataTableShowcase /></div>
  </div></section>;
}
function Why() {
  ...
  return <><section className="why section-pad section-grid" id="why-mayhem"><div className="shell why-grid"><div className="reveal"><p className="section-kicker">04 / Why Mayhem</p><h2>Less chasing.<br/><span className="gradient-text">More knowing.</span></h2><p>Most businesses do not need more software. They need fewer blind spots. Mayhem was built in Bangladesh for teams who are growing fast, carrying complexity, and done piecing together the truth.</p><a className="under-link" href="#contact">Explore the Mayhem approach <ArrowUpRight/></a></div><div className="principles">{principles.map(([num,title,copy]) => <article key={num}><span>{num}</span><div><h3>{title}</h3><p>{copy}</p></div></article>)}</div></div></section><section className="stats"><div className="shell"><article><span>◫</span><h3>1 workspace</h3><p>for the work that keeps your business moving.</p></article><article><span>⌾</span><h3>1 context</h3><p>from Dhaka to wherever your next chapter starts.</p></article><article><span>◇</span><h3>0 guesswork</h3><p>when the right people can see the right signal.</p></article></div></section></>;
}

function Contact() {
  return <section className="contact section-pad" id="contact"><div className="contact-backdrop"/><div className="shell reveal"><p className="section-kicker">05 / Start here</p><h2>Your next clear<br/><span className="gradient-text">decision starts here.</span></h2><p>Tell us where the work gets stuck. We will show you how Mayhem can connect it.</p><div className="button-row"><a className="button button-primary" href="mailto:info@mayhembangladesh.com?subject=Mayhem%20ERP%20demo">Contact us for a demo <ArrowUpRight/></a><a className="button button-outline" href="mailto:info@mayhembangladesh.com">info@mayhembangladesh.com</a></div></div></section>;
}

function Footer() {
  return <footer><div className="shell"><div><Brand/><p>A connected operations platform from Mayhem Softwares Bangladesh.</p><a className="footer-site" href="https://mayhembangladesh.com" target="_blank" rel="noreferrer">Visit Mayhem Bangladesh <ArrowUpRight/></a></div><nav><a href="#platform">Platform</a><a href="#demo">Product tour</a><a href="#contact">Contact us</a><span>© 2026 Mayhem Softwares</span></nav></div></footer>;
}
...
export default function App() {
  ...
  return <><Header/><main><Hero onVideo={() => setVideoOpen(true)} onImage={openImage}/><ProductTour/><Platform onImage={openImage}/><Workspace/><Why/><Contact/></main><Footer/><VideoModal open={videoOpen} onClose={() => setVideoOpen(false)}/><ImageModal image={modalImage} onClose={() => setModalImage(null)}/></>;
}