...
const workspaceFeatures = [
  ['01', 'Live operating view', 'See projects, stock, production, customers, and accounting signals in one place.'],
  ['02', 'Connected records', 'Keep every update linked so teams can move from detail to decision without losing context.'],
  ['03', 'Flexible workflows', 'Shape statuses, approvals, ownership, and handoffs around the way your business works.'],
  ['04', 'Role-based clarity', 'Give each team the views and actions they need while keeping one shared source of truth.'],
  ['05', 'Faster follow-through', 'Make priorities, responsibilities, and next steps visible before work begins to drift.'],
  ['06', 'Decision-ready reporting', 'Turn current operational activity into clear signals leaders can act on.'],
];
...
function Header() {
  ...
  return <header className={`site-header${scrolled ? ' scrolled' : ''}${menuOpen ? ' menu-open' : ''}`}>
    <div className="shell nav-shell"><Brand/><nav className="desktop-nav" aria-label="Primary navigation"><a href="#demo">Product tour</a><a href="#platform">Platform</a><a href="#screens">Workspace</a><a href="#why-mayhem">Why Mayhem</a></nav><div className="nav-actions"><a className="text-link" href="#contact">Contact us <span>↗</span></a><a className="button button-primary button-small" href="#contact">Request a demo <span>↗</span></a></div><button className="menu-toggle" type="button" aria-label={menuOpen ? 'Close menu' : 'Open menu'} aria-expanded={menuOpen} onClick={() => setMenuOpen(value => !value)}><span/><span/></button></div>
    <nav className={`mobile-nav${menuOpen ? ' open' : ''}`} aria-label="Mobile navigation"><a href="#demo" onClick={close}>Product tour</a><a href="#platform" onClick={close}>Platform</a><a href="#screens" onClick={close}>Workspace</a><a href="#why-mayhem" onClick={close}>Why Mayhem</a><a href="#contact" onClick={close}>Contact us</a><a className="mobile-cta" href="#contact" onClick={close}>Request a demo <span>↗</span></a></nav>
  </header>;
}
...
function Workspace() {
  return <section className="workspace section-pad" id="screens"><div className="shell">
    <div className="workspace-head reveal"><div><p className="section-kicker">03 / Inside the workspace</p><h2>Real work.<br/><span>No theater.</span></h2></div><p>Mayhem ERP is built for real operational work — not a concept, not a mockup.</p></div>
    <div className="workspace-features reveal">{workspaceFeatures.map(([number,title,copy]) => <article className="workspace-feature" key={number}><span>{number}</span><h3>{title}</h3><p>{copy}</p></article>)}</div>
  </div></section>;
}
function Why() {
